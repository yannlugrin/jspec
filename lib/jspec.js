...
;(function(){
  ...
  JSpec = {
    ...
    /**
     * Bind *real* global timer functions.
     */

    setTimeout    : bind(this.setTimeout, this),
    clearTimeout  : bind(this.clearTimeout, this),
    setInterval   : bind(this.setInterval, this),
    clearInterval : bind(this.clearInterval, this),
     ...
     */
     ...
     defaultContext : {
      ...
      /**
       * Halt execution until a certain amount of time has passed.
       * Note that the start time is defined when the first wait
       * is called, so multiple sleeps can be used in one block.
       *
       * @param {number} ms
       * @api public
       */
      ...
      sleep: function(time) {
        var start = null;
        JSpec.execution.wait(function() {
          start = start || Number(new Date);
          return Number(new Date) - start >= time
        })
      },

      /**
       * Defer function execution until a certain amount of time
       * has passed. Timeout defaults to 0ms, which means the
       * function is executed when the interpreter is idle.
       *
       * @params {function} fn
       * @params {number}   delay (optional)
       * @api public
       */

      defer: function(fn, delay) {
        JSpec.setTimeout(fn, delay || 0);
      },

      /**
       * Wait a certain amount of time before executing the assertions
      ...
      wait: function() {
        ...
        var delay = args.length == 2 ? parseInt(args.shift()) : 0;
        var fn = args.shift();
        var body = function() {
          var spec = JSpec.currentSpec
          var offset = spec.assertions.length
          fn.call(null)
          for (var i = offset; i < spec.assertions.length; i++) {
            spec.assertions[i].delay = delay;
            spec.assertions[i].run()
          }
        }
        body.once = true;
        body.delay = delay;
        JSpec.execution.wait(body);
      }
    },
    ...
    Assertion : function(matcher, actual, expected, negate) {
      extend(this, {
        ...
        run : function() {
          ...
          if (!this.passed) this.message = matcher.message.call(this, actual, expected, negate, matcher.name, this.delay)
          return this
        }
      })
    },
    ...
    Suite : function(description, body, isShared) {
      ...
      extend(this, {
        ...
        hook : function(hook, execution) {
					if (hook != 'before' && hook != 'after')
          	if (this.suite) this.suite.hook(hook, execution)

          each(this.hooks[hook], function(fn) {
            execution.insert(JSpec, 'runInContext', fn,
    ...
    ExecutionQueue: function() {
      var queue = []
      var waits = []
      var callbacks = []
      var index = 0

      this.insert = function(obj, prop) {
        if (typeof(obj) == 'function') {
          ...
          queue.push(function() {
            ...
            return obj[prop].apply(obj, curried.concat(args))
          })
        }
      }
      ...
      this.call = function(when, fn) {
        ...
        if (when & this.later) queue.push(fn)
      }
      ...
      function processWaits() {
        ...
        (function checker() {
          if (Number(new Date) - start > 2000) {
            alert('Timeout for function: ' + waits[0].toString())
            return
          }
          while(waits.length) {
            if (typeof(waits[0].delay) == 'number') {
              JSpec.setTimeout(checker, waits[0].delay)
              waits[0].delay = null
              return
            } else if (!waits[0].call(null) && !waits[0].once) {
              JSpec.setTimeout(checker, 10)
              return
            } else {
              waits.shift()
            }
    ...
    report : function() {
      this.execution.onComplete(bind(function() {
    ...
    run : function(options) {
      ...
      this.execution = new JSpec.ExecutionQueue;
    ...
    runSuite : function(suite) {
      ...
      if (!suite.isShared()) {
        ex.call(ex.now | ex.later, function() {
          JSpec.currentSuite = suite
          JSpec.runInContext(suite.fn)
          suite.ran = true
        })

        ex.insert(this, 'hook', 'beforeSuite', suite)
        suite.hook('before', ex)
        suite.hook('before_nested', ex)
        each(suite.specs, function(spec) {
          ex.insert(JSpec, 'hook', 'beforeSpec', spec)
          suite.hook('before_each', ex)
          ex.insert(JSpec, 'runSpec', spec)
          suite.hook('after_each', ex)
          ex.insert(JSpec, 'hook', 'afterSpec', spec)
        })
        if (suite.hasSuites()) {
        ...
        suite.hook('after', ex)
        suite.hook('after_nested', ex)
        ex.insert(this, 'hook', 'afterSuite', suite)
