...
;(function(){
  ...
  JSpec = {
      ...
      extend(this, {
        ...
        hook : function(hook) {
          ...
          each(this.hooks[hook], function(fn) {
            JSpec.runInContext(fn, "Error in hook '" + hook + "', suite '" + self.description + "': ")
          })
        },
    ...
    run : function(options) {
      ...
      var execution = new JSpec.ExecutionQueue;
      each(this.suites, function(suite) { JSpec.runSuite(suite, execution) })
      execution.process();
      return this
    },
    ...
    runSuite : function(suite, ex) {
      if (!suite.isShared()) {
        ...
        ex.insert(suite, 'hook', 'before')
        ex.insert(suite, 'hook', 'before_nested')
        each(suite.specs, function(spec) {
          ex.insert(JSpec, 'hook', 'beforeSpec', spec)
          ex.insert(suite, 'hook', 'before_each')
          ex.insert(JSpec, 'runSpec', spec)
          ex.insert(JSpec, 'hook', 'afterSpec', spec)
          ex.insert(suite, 'hook', 'after_each')
        })
        if (suite.hasSuites()) {
          each(suite.suites, function(suite) {
            JSpec.runSuite(suite, ex)
          })
        }
        ex.insert(this,  'hook', 'afterSuite', suite)
        ex.insert(suite, 'hook', 'after_nested')
        ex.insert(suite, 'hook', 'after')
