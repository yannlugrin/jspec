  ...
  JSpec = {
     ...
     */
     ...
     defaultContext : {
      ...
      /**
       * Expecting _n_ assertions before finishing the spec.
       *
       * Options:
       *  - timeout:  timeout in milliseconds defaulting to 500
       *
       * @param  {number} n
      ...
      expecting: function(n, options) {
        options = options || {}
        function fn(){ return n === JSpec.currentSpec.assertions.length }
        fn.n = n
        if (options.timeout) fn.timeout = options.timeout
        JSpec.queue.wait(fn)
      }
    },
        ...
        return (function loop() {
          while (waits.length)
            if (typeof waits[0].delay === 'number') {
              ...
              if (waits[0].timeout)
                JSpec.setTimeout(function(){
                  if (!waits[0].call())
                    fail('expecting(' + waits[0].n + ') timed out after ' + waits[0].timeout + 'ms')
                }, waits[0].timeout)
              sync = false
              return
            } else
