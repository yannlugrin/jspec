...
;(function(){
  ...
  /**
   * Bind _fn_ to the given _context_.
   *
   * @param  {function} fn
   * @param  {mixed} context
   * @return {function}
  ...
  JSpec = {
    ...
    /**
     * Reference original timers to allow the mock timers
     * library to override the globals.
     */

    setTimeout    : bind(setTimeout, this),
    clearTimeout  : bind(clearTimeout, this),
    setInterval   : bind(setInterval, this),
    clearInterval : bind(clearInterval, this),
     ...
     */
     ...
     defaultContext : {
      ...
      /**
       * Halt execution for n _ms_; may be used several times
       * in a single block.
       *
       * @param {number} ms
       * @api public
      ...
      sleep: function(ms) {
        var start
        JSpec.queue.wait(function() {
          start = start || Number(new Date)
          return Number(new Date) - start >= ms
        })
      },

      /**
       * Wait n _ms_ before executing _fn_ which may
       * contain assertions.
       *
       * @params {number} ms
       * @params {function} fn
      ...
      wait: function(ms, fn) {
        ...
        function body() {
          start = start || Number(new Date)
          var spec = JSpec.currentSpec,
              offset = spec.assertions.length,
              fail = false,
              timeout = Number(new Date) - start > ms
          fn.call()
          for (var i = offset; i < spec.assertions.length; ++i)
            spec.assertions[i].delay = ms,
            spec.assertions[i].run(),
            hook('afterAssertion', spec.assertions[i]),
            fail = fail || !spec.assertions[i].passed
          if (fail && timeout)
            for (var i = offset; i < spec.assertions.length; ++i)
              spec.assertions[i].report()
          else if (fail && !timeout)
            spec.assertions.splice(offset)
          return !fail || timeout
        }
        JSpec.queue.wait(body)
      }
    },
    ...
    Assertion : function(matcher, actual, expected, negate) {
      extend(this, {
        ...
        report : function() {
          if (!this.passed)
            this.message = matcher.message.call(this, actual, expected, negate, matcher.name, this.delay)
          if (JSpec.assert) 
        ...
        run : function() {
          this.result = matcher.match.apply(this, [actual].concat(expected))
    ...
    Suite : function(description, body, isShared) {
      ...
      extend(this, {
        ...
        addHook : function(hook, body) {
          if (this.ran) return;
          this.hooks[hook].push(JSpec.wrapInContext(body))
        },
        ...
        hook : function(hook, queue) {
					if (hook !== 'before' && hook !== 'after')
          	if (this.suite)
          	  this.suite.hook(hook, queue)

          each(this.hooks[hook], function(fn) {
            queue.push(JSpec, 'runInContext', fn,
              "Error in hook '" + hook + "', suite '" + self.description + "': ")
          })
        },
        ...
        // Check if a suite is a shared behavior

				isShared : function(){
					return this.shared
				},
				
				// Check if a suite is not shared, and contains specs
    ...
    Spec : function(description, body) {
      extend(this, {
        ...
        // Report message for every assertion

        reportAssertions : function() {
    ...
    /**
     * Builds an execution queue of functions to be sequentially
     * executed, so we can pause and resume at any time.
     */
    ...
    Queue: function() {
      ...
      this.push = function(obj, prop) {
        if (typeof obj === 'function') {
          stack.push(obj)
        } else {
          var args = toArray(arguments, 2)
          stack.push(function() {
            return obj[prop].apply(obj, args.concat(arguments))
          })
        }
      }
      ...
      this.now = 1
      this.later = 2
      this.call = function(when, fn) {
        if (when & this.now) fn.call()
        if (when & this.later) stack.push(fn)
      }

      this.start = function() {
        index = 0
        JSpec.setTimeout(resume, 0)
      }
      ...
      // Return true if all the wait functions could be succesfully
      // executed synchronously. This causes resume() to continue
      // processing instead of returning.
      ...
      function processWaits() {
        ...
        return (function loop() {
          while(waits.length)
            if (typeof waits[0].delay === 'number') {
              JSpec.setTimeout(loop, waits[0].delay)
              waits[0].delay = null
              sync = false
              return
            } else if (!waits[0].call() && !waits[0].once) {
              JSpec.setTimeout(loop, 10)
              sync = false
              return
            } else
              waits.shift()
    ...
    defaultMatcherMessage : function(actual, expected, negate, name, delay) {
      return 'expected ' + puts(actual) + ' to ' + 
               (negate ? 'not ' : '') + 
                  name.replace(/_/g, ' ') +
                    (expected.length > 0 ? ' ' + puts.apply(this, expected) : '') +
                      (delay ? (' within ' + delay + 'ms') : '')
    },
    ...
    expect : function(actual) {
      function assert(matcher, expected, negate) {
        matcher.negate = negate  
        assertion = new JSpec.Assertion(matcher, actual, expected, negate)
        hook('beforeAssertion', assertion)
        if (matcher.defer) assertion.run()
        else JSpec.currentSpec.assertions.push(assertion.run()), hook('afterAssertion', assertion)
        return assertion.result
      }
      
      function to(matcher) {
        return assert(matcher, toArray(arguments, 1), false)
      }
      
      function not_to(matcher) {
        return assert(matcher, toArray(arguments, 1), true)
      }
    ...
    /**
     * Wraps a capture body in a function
     * such that the execution scope can be
     * defined at run-time
     *
     * @param  {function} body
     * @return {function}
    ...
    wrapInContext : function(body) {
      var contents = this.contentsOf(body);
      return eval("(function(context) { with(context) { " + contents + " } })");
    },

    /**
     * Executes the given function in the scope
     * used for executing specs.
     *
     * @param  {function} fn
     * @api private
     */

    runInContext : function(fn, errorMessage) {
      var context = {};
      this.extend(context, this.DSL || this.DSLs.snake);
      this.extend(context, this.context || this.defaultContext);
      this.extend(context, this.matchers);
    ...
    report : function() {
      this.queue.onComplete(bind(function() {
    ...
    runSuite : function(suite) {
      ...
      if (suite.isShared()) return
      queue.call(queue.now | queue.later, function() {
        JSpec.currentSuite = suite
        JSpec.runInContext(suite.fn)
        suite.ran = true
      })

      queue.push(this, 'hook', 'beforeSuite', suite)
      suite.hook('before', queue)
      suite.hook('before_nested', queue)
      each(suite.specs, function(spec) {
        queue.push(JSpec, 'hook', 'beforeSpec', spec)
        suite.hook('before_each', queue)
        queue.push(JSpec, 'runSpec', spec)
        suite.hook('after_each', queue)
        queue.push(JSpec, 'hook', 'afterSpec', spec)
      })
      if (suite.hasSuites()) {
      ...
      suite.hook('after', queue)
      suite.hook('after_nested', queue)
      queue.push(this, 'hook', 'afterSuite', suite)
    ...
    runSpec : function(spec) {
      ...
      try { this.runInContext(spec.fn) }
      catch (e) { fail(e) }
      spec.runDeferredAssertions()
      spec.reportAssertions()
      destub()
  ...
  addMatchers({
    ...
    throw_error : { match : function(actual, expected, message) {
      ...
      exception = message_for(0) + (expected[1] ? ' and ' + message_for(1) : '')
